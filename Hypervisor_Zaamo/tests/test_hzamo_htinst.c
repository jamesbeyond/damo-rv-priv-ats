...
/* ===================================================================
 * Group 2.4 - htinst: transformed atomic instruction vs pseudoinstruction
 *             (HZAMO-15 ~ HZAMO-20)
 *
 * Spec basis:
 *   norm:H_trap_xtinst_val / htinst_transformed_atomic - for an explicit
 *     AMO access fault, htinst is 0 or the transformed atomic instruction
 *     (all fields preserved except bits 19:15 = Addr.Offset).
 *   norm:H_vm_gpapriv - an implicit VS-stage page-table walk fault is
 *     reported for the AMO's ORIGINAL access type -> store/AMO guest-page
 *     fault (cause 23), NOT LR's cause 21.
 ...
 *   AMO atomicity => a single memory operation => Addr. Offset == 0.
 * =================================================================== */

static inline uintptr_t hzamo_pte_gpa_in_pt(uintptr_t pt_page_base,
...
/* ------------------------------------------------------------------
 * HZAMO-15: guest AMO explicit G-stage fault -> htinst = 0 or transformed
 *           atomic (funct5 = amoadd 00000).
 * ------------------------------------------------------------------ */
TEST_REGISTER(test_hzamo_15_amo_htinst_transformed);
bool test_hzamo_15_amo_htinst_transformed(void)
{
    ...
    if (fired)
        check_xtinst_zero_or_golden("AMO htinst transformed-atomic", 0x2FUL, va);
    if (htinst != 0)
        TEST_ASSERT_EQ("nonzero htinst funct5 == amoadd (00000)",
                       (htinst >> 27) & 0x1FUL, (uintptr_t)0x00UL);
...
/* ------------------------------------------------------------------
 * HZAMO-16: htinst preserves the aq (bit26) / rl (bit25) annotations.
 * ------------------------------------------------------------------ */
static void hzamo_aqrl_case(uintptr_t (*probe)(uintptr_t), uintptr_t va,
                            uintptr_t g_flags, unsigned exp_aq, unsigned exp_rl)
{
    ...
    trap_expect_end();
    if (fired)
        check_xtinst_zero_or_golden("aq/rl AMO htinst", 0x2FUL, va);
    ts2_finish(&ctx);

    TEST_ASSERT("aq/rl AMO variant faulted", fired);
    if (htinst != 0) {
...
TEST_REGISTER(test_hzamo_16_htinst_aqrl);
bool test_hzamo_16_htinst_aqrl(void)
{
    ...
    hzamo_aqrl_case(hz_vs_amo_add_w_rl, va, HZ_G_RU, 0, 1);
    hzamo_aqrl_case(hz_vs_amo_add_w_aqrl, va, HZ_G_RU, 1, 1);

    HYP_TEST_END();
}

/* ------------------------------------------------------------------
 * HZAMO-17: AMO htinst Addr. Offset (bits 19:15) is always 0.
 * ------------------------------------------------------------------ */
TEST_REGISTER(test_hzamo_17_addr_offset_zero);
...
/* ------------------------------------------------------------------
 * HZAMO-18: implicit VS-stage walk fault -> cause 23 (AMO original type,
 *           NOT LR's 21) + read pseudoinstruction when htval != 0.
 * ------------------------------------------------------------------ */
TEST_REGISTER(test_hzamo_18_implicit_walk_cause23);
bool test_hzamo_18_implicit_walk_cause23(void)
{
    ...
    TEST_ASSERT("implicit-walk AMO fault fired", fired);
    /* norm:H_vm_gpapriv: reported for the AMO's original (store/AMO) type. */
    TEST_ASSERT_EQ("cause == store/AMO guest-page-fault (23), NOT 21",
                   cause, (uintptr_t)CAUSE_STORE_GUEST_PAGE_FAULT);
    uintptr_t exp_htval = hzamo_pte_gpa_in_pt(pt_gpa, va, 0) >> 2;
...
/* ------------------------------------------------------------------
 * HZAMO-19: ADUE=1, VS PTE D=0 (AMO writes) + PT page G-stage read-only
 *           -> implicit D-update store faults: write pseudoinst, cause 23.
 * ------------------------------------------------------------------ */
TEST_REGISTER(test_hzamo_19_adue_write_pseudo);
bool test_hzamo_19_adue_write_pseudo(void)
{
    ...
    *pte = ppn | PTE_V | PTE_R | PTE_W | PTE_X | PTE_A;   /* D=0 */
    ...
    printf("  [INFO] ADUE AMO D-update: fired=%d cause=%lu htval=0x%lx "
           "htinst=0x%lx\n", (int)fired, (unsigned long)cause,
           (unsigned long)htval, (unsigned long)htinst);
    TEST_ASSERT("A/D-update AMO fault fired", fired);
    TEST_ASSERT_EQ("cause == store/AMO guest-page-fault (23)",
                   cause, (uintptr_t)CAUSE_STORE_GUEST_PAGE_FAULT);
    if (htval != 0) {
...
/* ------------------------------------------------------------------
 * HZAMO-20: explicit vs implicit fault disambiguation (same cause=23).
 * ------------------------------------------------------------------ */
TEST_REGISTER(test_hzamo_20_explicit_vs_implicit);
bool test_hzamo_20_explicit_vs_implicit(void)
{
    ...
    TEST_ASSERT("(a) explicit AMO fault fired", fired_a);
    TEST_ASSERT_EQ("(a) cause == 23", cause_a,
                   (uintptr_t)CAUSE_STORE_GUEST_PAGE_FAULT);
    TEST_ASSERT("(a) htinst == 0 or transformed (funct5=amoadd)",
                htinst_a == 0 || ((htinst_a >> 27) & 0x1FUL) == 0x00UL);
    ...
    trap_expect_begin();
    (void)two_stage_run_in_vs(&ctx, hz_vs_amo_add_w, va_b);
    ...
    TEST_ASSERT("(b) implicit-walk AMO fault fired", fired_b);
    TEST_ASSERT_EQ("(b) cause == 23 (AMO original type)", cause_b,
                   (uintptr_t)CAUSE_STORE_GUEST_PAGE_FAULT);
    if (htval_b != 0) {
        uintptr_t exp = hzamo_pte_gpa_in_pt(pt_gpa, va_b, 0) >> 2;
